...
It listens on `0.0.0.0:8555` and needs about 56 MB of memory. On start it replays the chain from disk; blocks at or below the newest checkpoint skip the scrypt proof of work and the signature checks, because the checkpoint already settles what that history is and every block names its parent, so a changed byte breaks a link. Set `DONUTCOIN_VERIFY_ALL=1` to verify every block from genesis anyway. The chain is 2.2 MB for the first
