...
def test_longer_chain_with_more_work_replaces(tmp_path):
    a, b = new_keypair(), new_keypair()                     # different payees, or the two chains build identical blocks
    ours = Chain(str(tmp_path / "a")); ours.add_block(make_block(ours, a[2]))
    theirs = Chain(str(tmp_path / "b"))
    for _ in range(3): theirs.add_block(make_block(theirs, b[2]))
    assert ours.replace_with(theirs.blocks[1:]) is True and ours.height == 3 and ours.tip.hash == theirs.tip.hash
    assert [(r["height"], r["depth"], r["added"]) for r in ours.reorgs] == [(1, 1, 3)]   # forked at genesis: lost 1, took 3


def test_reorg_is_counted_only_when_the_chain_actually_changes(tmp_path, capsys):
    a, b = new_keypair(), new_keypair()
    ours = Chain(str(tmp_path / "a"))
    for _ in range(2): ours.add_block(make_block(ours, a[2]))
    theirs = Chain(str(tmp_path / "b")); theirs.add_block(make_block(theirs, b[2]))
    assert ours.replace_with(theirs.blocks[1:]) is False and ours.reorgs == []
    for _ in range(2): theirs.add_block(make_block(theirs, b[2]))
    assert ours.replace_with(theirs.blocks[1:]) is True
    assert len(ours.reorgs) == 1 and ours.reorgs[0]["depth"] == 2 and "reorg: 2 block(s) replaced from height 1" in capsys.readouterr().out
...
def test_catching_up_is_not_a_reorg(tmp_path, capsys):
    a = new_keypair()
    ours = Chain(str(tmp_path / "a")); ours.add_block(make_block(ours, a[2]))
    theirs = Chain(str(tmp_path / "b")); theirs.add_block(ours.blocks[1])     # same history as ours...
    for _ in range(3): theirs.add_block(make_block(theirs, a[2]))           # ...and further along
    assert ours.replace_with(theirs.blocks[1:]) is True and ours.height == 4
    out = capsys.readouterr().out
    assert ours.reorgs == [] and "reorg" not in out and "caught up: 3 block(s) adopted from height 2" in out
...
def _pinned_chain(tmp_path, monkeypatch, n=8):
    """A persisted chain with a spend in it, and a checkpoint pinned partway up."""
    import donutcoin.chain as C
    ch = C.Chain(str(tmp_path))
    kp = new_keypair()
    for _ in range(n): ch.add_block(make_block(ch, kp[2]))
    ch.add_block(make_block(ch, kp[2], txs=[spend(ch, kp, kp[2], 5000)]))   # real signatures to skip
    return C, ch, kp


def test_trusted_load_below_a_checkpoint_gives_identical_state(tmp_path, monkeypatch):
    """Skipping the two cryptographic proofs below a checkpoint must change speed and nothing
    else: same tip, same UTXO set, same work."""
    C, ch, _ = _pinned_chain(tmp_path, monkeypatch)
    want = (ch.height, ch.tip.hash, dict(ch.utxo), ch.work())

    monkeypatch.setattr(C, "CHECKPOINTS", {5: ch.blocks[5].hash})
    trusted = C.Chain(str(tmp_path))
    assert (trusted.height, trusted.tip.hash, trusted.utxo, trusted.work()) == want

    monkeypatch.setattr(C, "VERIFY_ALL", True)                    # the opt-out still works
    full = C.Chain(str(tmp_path))
    assert (full.height, full.tip.hash, full.utxo, full.work()) == want


def test_tampering_below_a_checkpoint_still_breaks_the_hash_chain(tmp_path, monkeypatch):
    """Trust rests on the linkage: every block names its parent, so a changed byte anywhere below
    the checkpoint breaks a link even though nothing re-runs the proof of work."""
    import json
    C, ch, _ = _pinned_chain(tmp_path, monkeypatch)
    monkeypatch.setattr(C, "CHECKPOINTS", {5: ch.blocks[5].hash})
    path = ch.path
    ...
    d = json.loads(lines[2]); d["timestamp"] += 1                 # block 3, well below the checkpoint
    lines[2] = json.dumps(d)
    open(path, "w").write("\n".join(lines) + "\n")

    with pytest.raises(ValidationError):
        C.Chain(str(tmp_path))


def test_signatures_above_the_checkpoint_are_still_checked(tmp_path, monkeypatch):
    """A signature is not part of the txid, so corrupting one leaves every block hash intact and
    the linkage cannot catch it. Above the checkpoint that is exactly what verification is for."""
    import json
    C, ch, _ = _pinned_chain(tmp_path, monkeypatch)
    monkeypatch.setattr(C, "CHECKPOINTS", {5: ch.blocks[5].hash})
    path = ch.path
    ...
    lines = open(path).read().splitlines()
    d = json.loads(lines[-1])                                     # the spend, above the checkpoint
    sig = d["txs"][1]["inputs"][0]["signature"]
    d["txs"][1]["inputs"][0]["signature"] = ("0" if sig[0] != "0" else "1") + sig[1:]
...
def test_a_coin_claimed_by_a_waiting_transaction_is_not_spendable(funded):
    """The 2026-09-26 sweep: a note was waiting to spend @donut's biggest coin, the address listing
    still offered that coin, and the sweep built a second spend of it, which was refused."""
    ch, alice = funded
    before = ch.spendable(alice[2]); assert len(before) == 1
    ch.add_to_mempool(spend(ch, alice, new_keypair()[2], COIN))
    assert ch.spendable(alice[2]) == []            # still unspent on the chain, but already claimed
