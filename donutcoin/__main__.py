...
def main():
    ...
    if cmd == "note":
        from .wallet import main as m; sys.argv = ["wallet", "note"] + rest + ([] if any(r.startswith("--node") for r in rest) else ["--node", NODE]); return m()
    if cmd == "wallet":
        from .wallet import main as m; sys.argv = ["wallet"] + rest; return m()
    if cmd == "hide":
        from .hide import main as m; sys.argv = ["hide"] + rest; return m()
    if cmd == "node":
        from .node import main as m; sys.argv = ["node"] + rest; return m()
    print("usage: donutcoin start [--to @username|D-address] [--threads N] | miner ... | wallet new|address|balance|send|claim ... | note \"text\" | node"); sys.exit(2)
