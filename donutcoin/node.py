...
from .chain import Chain, Block, ValidationError, BLOCK_REWARD, TARGET_SPACING, RETARGET_INTERVAL, COINBASE_MATURITY, MAX_BLOCK_TXS
...
def download():
    ...
    rows = [("donutcoin-macos-arm64", "Mac, Apple silicon (M1 and later)", "Right-click the file, choose Open, then Open again. Once."),
            ("donutcoin-macos-intel", "Mac, Intel", "Right-click the file, choose Open, then Open again. Once."),
    ...
    for name, label, note in rows:
        ...
        if a:
            ...
            items += f'<div class="dl"><div><b>{label}</b><div class="muted">{note} {a["size"] // 1048576} MB.</div>{sha}</div><a href="{a["browser_download_url"]}"><button>Download</button></a></div>'
        else:
    ...
    src = f'<a href="https://github.com/nyc-esq/donutcoin/tree/{version}">source at {version}</a>' if rel.get("tag_name") else ""
