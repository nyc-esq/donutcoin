...
from .chain import Chain, Block, ValidationError, BLOCK_REWARD, TARGET_SPACING, RETARGET_INTERVAL, COINBASE_MATURITY, MAX_BLOCK_TXS, DEAD_ADDRESSES, MAX_NOTE_CHARS, NOTE_RULES_HEIGHT, NOTE_MIN_FEE, NOTE_LICENCE_BLOCKS
...
RULES = {"notes_from": NOTE_RULES_HEIGHT}                     # consensus knobs this node runs with; a peer's differ = a rule change
...
def notes(limit: int = Query(50, le=500)):
    ...
    while h >= 0 and len(out) < limit:
        ...
        h -= 1
    return {"max_chars": MAX_NOTE_CHARS, "min_fee": NOTE_MIN_FEE, "rules_from": NOTE_RULES_HEIGHT, "licence_blocks": NOTE_LICENCE_BLOCKS, "notes": out}
...
def tx(txid: str):
    ...
    h, pos = loc
    return {"status": "confirmed", "height": h, "confirmations": chain.height - h + 1, **scrub(chain.blocks[h].txs[pos].to_dict())}
