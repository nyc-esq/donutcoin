...
def explorer():
    with open(os.path.join(STATIC, "explorer.html")) as f:
        return HTMLResponse(f.read(), headers=HTML_HEADERS)
...
def page(page: str):
    ...
    with open(os.path.join(STATIC, PAGES[page])) as f:
        return HTMLResponse(f.read(), headers=HTML_HEADERS)
...
def note_page(txid: str):
    ...
    with open(os.path.join(STATIC, "note.html")) as f: page = f.read()
...
def who_page(key: str):
    ...
    with open(os.path.join(STATIC, "who.html")) as f: page = f.read()
