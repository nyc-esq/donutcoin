...
def main():
    ...
    if a.cmd in ("send", "note"):
        ...
        try:
            with urllib.request.urlopen(req, timeout=120) as r: print("noted" if a.cmd == "note" else "sent", json.load(r)["txid"])   # a node checks every signature before it answers: 1,500 inputs took a small public node over 10 s (2026-09-26)
