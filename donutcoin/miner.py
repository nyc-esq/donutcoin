...
"""
import argparse, json, multiprocessing as mp, os, sys, time, urllib.request
...
def main():
    ...
    ap.add_argument("--node", default="http://127.0.0.1:8555"); ap.add_argument("--address", required=True)
    ap.add_argument("--threads", type=int, default=max(1, mp.cpu_count() // 2)); ap.add_argument("--tag", default="donut")
    a = ap.parse_args()
    print(f"donutcoin miner: {a.threads} workers -> {a.node}, paying {a.address}", flush=True)
    total, t0, blocks = 0, time.time(), 0
    while True:
        try:
            tmpl = get(f"{a.node}/api/template?address={a.address}")
